...
namespace FT.GerenciamentoProcessosJudiciais.TestesIntegracao
{
    static class ApiWrapper
    {
        ...
        public static async Task<dynamic> GraphQLAsync(this string query, dynamic variables = null, string operationName = null)
        {
            ...
            using var client = new GraphQLClient($"{ApiUrl}/graphql");
            var response = await client.PostAsync(request);
            if (response.Errors?.Length > 0) throw new InvalidOperationException(response.Errors.ToString());
